...
public class Graph {
	...
	public void shortestPath(Vertice startVertice, Vertice endVertice) throws FileNotFoundException {
		...
		LinkedList<Vertice> queue = new LinkedList<Vertice>(); // Creating queue in order to visit vertices in correct
																// order
		queue.add(startVertice); // Adding start vertex to queue

		while (!queue.isEmpty()) { // As long as there are vertices in the queue
			queue = quickSortOfVertice(queue,0,queue.size()); // Sorting queue based on distance (smallest distance first)
			if (queue.get(0).getVisited() == false) { // If first element in queue is not visited
				for (int i = 0; i < queue.get(0).getAdjacents().size(); i++) {// For all the vertex neighbors
					...
					int newDistance = queue.get(0).getDistance() + queue.get(0).getLengthToAdjacents().get(i); // Calculating
																												// distance
																												// of
																												// neighbor

					for (int j = 0; j < vertices.size(); j++) { // Searching for itself in "vertices"
						if (queue.get(0).getAdjacents().get(i).getNodeId() == vertices.get(j).getNodeId()) { // Found
																												// itself
							if (newDistance < vertices.get(j).getDistance()) {// If its new distance is shorter than its
																				// current
		...
		System.out.println("\nShortest distance between " + vertices.get(startVertice.getNodeId()).getName() + " and "
				+ vertices.get(endVertice.getNodeId()).getName() + " is "
				+ vertices.get(endVertice.getNodeId()).getDistance());
